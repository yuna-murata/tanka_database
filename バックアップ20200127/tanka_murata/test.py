...
def hello():
    ...
    hit_poet_num=""
    html =render_template("index.html",hit_author=hit_author,pagination=pagination,y_labels_list=y_labels_list,bar_labels=bar_labels,graph_data_list=graph_data_list,backgroundColor_list=backgroundColor_list,hit_poet_num=hit_poet_num)
    return html
...
def hello_1():
   ...
   #Apatch sorlに問い合わせるURLを作成
    base_url = 'http://127.0.0.1:8983/solr/tanka4/select?q='
    #'http://127.0.0.1:8983/solr/tanka4/select?q='　ローカルで動かすURL
    # 'http://54.168.222.70:8983/solr/mycore/select?q=' apache で動かすURL
...
#モデルファイルの選択
    file_name= "/var/www/models/"+"model_s"+get_size+"_minc"+get_min_count+"_win"+get_window+".model"
    #"model_s"+get_size+"_minc"+get_min_count+"_win"+get_window+".model" ローカルで動かす際のモデルファイルパス
#モデルのよみこみ
